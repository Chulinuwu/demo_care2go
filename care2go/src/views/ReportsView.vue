...
<template>
  <div class="space-y-4 md:space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 lg:gap-6">
      <div class="bg-white rounded-lg shadow p-4 md:p-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div>
            <div class="text-xs md:text-sm text-gray-600 mb-1">การจองทั้งหมด</div>
            <div class="text-2xl md:text-3xl font-bold text-gray-800">{{ totalAppointments }}</div>
          </div>
      ...
      <div class="bg-white rounded-lg shadow p-4 md:p-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div>
            <div class="text-xs md:text-sm text-gray-600 mb-1">เสร็จสิ้น</div>
            <div class="text-2xl md:text-3xl font-bold text-green-600">{{ completedAppointments }}</div>
          </div>
      ...
      <div class="bg-white rounded-lg shadow p-4 md:p-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div>
            <div class="text-xs md:text-sm text-gray-600 mb-1">รายได้รวม</div>
            <div class="text-xl md:text-2xl font-bold text-indigo-600">{{ (totalRevenue / 1000000).toFixed(2) }}M</div>
          </div>
      ...
      <div class="bg-white rounded-lg shadow p-4 md:p-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div>
            <div class="text-xs md:text-sm text-gray-600 mb-1">ผู้ดูแลพร้อมงาน</div>
            <div class="text-2xl md:text-3xl font-bold text-teal-600">{{ activeCaregivers }}/{{ totalCaregivers }}</div>
          </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
      <!-- Top Services -->
      <div class="bg-white rounded-lg shadow p-4 md:p-6">
        <h2 class="text-lg md:text-xl font-semibold text-gray-800 mb-4 md:mb-6">บริการยอดนิยม (Top 5)</h2>
        <div class="space-y-3 md:space-y-4">
          <div v-for="(service, index) in topServices" :key="service.id" class="flex items-center gap-2 md:gap-4">
            <div class="text-base md:text-lg font-semibold text-gray-600 w-5 md:w-6">{{ index + 1 }}</div>
            <div class="flex-1">
      ...
      <div class="bg-white rounded-lg shadow p-4 md:p-6">
        <h2 class="text-lg md:text-xl font-semibold text-gray-800 mb-4 md:mb-6">การจองตามหมวดหมู่</h2>
        <div class="space-y-3 md:space-y-4">
    ...
    <!-- Export Section -->
    <div class="bg-white rounded-lg shadow p-4 md:p-6">
      <h2 class="text-lg md:text-xl font-semibold text-gray-800 mb-4">ส่งออกรายงาน</h2>
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
        <button class="flex items-center justify-center gap-2 bg-green-50 hover:bg-green-100 text-green-700 font-semibold py-2.5 md:py-3 px-4 md:px-6 rounded-lg transition text-sm md:text-base">
